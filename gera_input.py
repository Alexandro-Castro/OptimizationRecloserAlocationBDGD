...
for alimentador in alimentadores:
    ...
    nos['CHAVE'] = np.where(nos['TIPO'] == 'CP', 0, 1)
    print(nos)

    nos_ot = nos.groupby('PAC_1').agg(
        TRONCO=('TRONCO', 'max'),
        CHAVE=('CHAVE','max'),
        UCs_A_JUS=('UCs_A_JUS', 'max'),
        UCs_B_JUS=('UCs_B_JUS', 'max'),
        DIC_JUS=('DIC_JUS', 'max'),
        FIC_JUS=('FIC_JUS', 'max'),
        DIST_RAIZ=('DIST_RAIZ', 'max')
    ).reset_index()

    nos_ot.to_csv(fr'{caminho}\otimizar_{alimentador}.csv',sep=';')
